...
class U_Net(nn.Module):
    def __init__(self, img_ch=3, output_ch=1):
        ...
        self.drop1 = nn.Dropout2d(p=0.2)  # 添加 Dropout
    ...
    def forward(self, x):
        ...
        d5 = self.Up5(x5)
        d5 = self.drop_up5(d5)  # 上采样后 Dropout
...
class AttU_Net(nn.Module):
    def __init__(self,img_ch=3,output_ch=1):
        ...
        # 编码器部分添加Dropout
        self.Conv1 = conv_block(ch_in=img_ch,ch_out=32)
        self.drop1 = nn.Dropout2d(p=0.2)  # 第一个Dropout
        ...
        # 解码器部分添加Dropout
        self.Up5 = up_conv(ch_in=512,ch_out=256)
        self.drop_up5 = nn.Dropout2d(p=0.2)  # 上采样后的Dropout
        self.Att5 = Attention_block(F_g=256,F_l=256,F_int=128)
        self.Up_conv5 = conv_block(ch_in=512, ch_out=256)

        self.Up4 = up_conv(ch_in=256,ch_out=128)
        self.drop_up4 = nn.Dropout2d(p=0.15)  # 逐渐降低dropout概率
    ...
    def forward(self,x):
        ...
        # decoding + concat path
        d5 = self.Up5(x5)
        d5 = self.drop_up5(d5)  # 上采样后dropout
