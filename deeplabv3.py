...
class Xception(nn.Module):
    ...
    def _load_pretrained_model(self):
        ...
        for k, v in pretrained_weights.items():
            if k in state_dict:
                # 处理conv1的通道不匹配问题（3通道→1通道）
                if k == 'conv1.weight' and v.size(1) != state_dict[k].size(1):
                    # 取3个通道的平均值作为1通道的权重
                    v = v.mean(dim=1, keepdim=True)  # 从[32,3,3,3]变为[32,1,3,3]
    ...
    def __init__(self, num_classes, in_channels=3, backbone='xception', pretrained=True, 
        ...
        if 'resnet' in backbone:
            ...
            self.backbone = Xception(
                ...
                in_channels=in_channels,  # 添加这一行，传递输入通道数
